...
class GenerateSummaryJob implements ShouldQueue
{
    ...
    public function handle(GeminiAIService $aiService): void
    {
        ...
        if ($aiContent->status !== 'processing') {
            Log::info("Skipping summary generation for AI content {$this->aiContentId} - status is {$aiContent->status}");
            return;
        }

        try {
            Log::info("Starting summary generation for AI content {$this->aiContentId}");
            ...
            $summaryType = $parameters['summary_type'] ?? 'concise';
            $maxWords = $parameters['max_words'] ?? 300;

            // Generate the summary
            $summary = $aiService->generateSummaryFromFile(
                $attachmentPath,
                $mimeType,
                $summaryType,
                $maxWords
            ...
            // Mark as completed
            $aiContent->markAsCompleted($summary);

            Log::info("Successfully completed summary generation for AI content {$this->aiContentId}");

        } catch (\Exception $e) {
            Log::error("Failed to generate summary for AI content {$this->aiContentId}", [
                'error' => $e->getMessage(),
    ...
    public function failed(\Throwable $exception): void
    {
        ...
        Log::error("Summary generation job failed permanently for AI content {$this->aiContentId}", [
